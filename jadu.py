...
# Set the API key
#os.environ['PANDASAI_API_KEY'] = '$2a$10$jsZ5reeJtKlAeI7ekHaPTu76x5LrUv/KyhPJbkx7dINw3pI02ZkUW'
# Initialize the LLM model
model = LocalLLM(api_base="http://localhost:11434/v1", model="gemma2:2b")
...
pdf_path = "docs/user_guide.pdf"  # Path to the PDF inside the 'doc' folder
    ...
    if uploaded_file is not None:
        ...
        if input_text:
            if st.button("Chat with CSV"):
                ...
                # Using PandasAI to handle the user's query
                pandas_ai = SmartDataframe(st.session_state.processed_data)
        ...
        # Detect what kind of plot is needed, e.g., heatmap, bar plot, etc.
            
                if "heatmap" in input_text.lower():
                    plt.figure(figsize=(10, 6))
                    sns.heatmap(st.session_state.processed_data.corr(), annot=True, cmap='coolwarm')
                    st.pyplot(plt)

                elif "bar" in input_text.lower():
                    column_name = input_text.split()[-1]  # Assume the last word is the column name
                    plt.figure(figsize=(10, 4))
                    sns.barplot(x=st.session_state.processed_data[column_name], y=st.session_state.processed_data.index)
                    st.pyplot(plt)

                elif "scatter" in input_text.lower():
                    x_column = input_text.split()[-2]  # Assume the second last word is the x column name
                    y_column = input_text.split()[-1]  # Assume the last word is the y column name
                    plt.figure(figsize=(10, 6))
                    sns.scatterplot(x=st.session_state.processed_data[x_column], y=st.session_state.processed_data[y_column])
                    st.pyplot(plt)

                elif "line" in input_text.lower():
                    x_column = input_text.split()[-2]  # Assume second last word is the x column
                    y_column = input_text.split()[-1]  # Assume last word is the y column
                    plt.figure(figsize=(10, 6))
                    sns.lineplot(x=st.session_state.processed_data[x_column], y=st.session_state.processed_data[y_column])
                    st.pyplot(plt)

                elif "box" in input_text.lower():
                    column_name = input_text.split()[-1]  # Assume the last word is the column name
                    plt.figure(figsize=(10, 6))
                    sns.boxplot(y=st.session_state.processed_data[column_name])
                    st.pyplot(plt)

                elif "histogram" in input_text.lower() or "hist" in input_text.lower():
                    column_name = input_text.split()[-1]  # Assume the last word is the column name
                    plt.figure(figsize=(10, 6))
                    sns.histplot(st.session_state.processed_data[column_name], bins=30, kde=False)
                    st.pyplot(plt)

                elif "distplot" in input_text.lower() or "distribution" in input_text.lower():
                    column_name = input_text.split()[-1]  # Assume the last word is the column name
                    plt.figure(figsize=(10, 6))
                    sns.histplot(st.session_state.processed_data[column_name], kde=True)
                    st.pyplot(plt)

                elif "pie" in input_text.lower():
                    column_name = input_text.split()[-1]  # Assume the last word is the column name
                    pie_data = st.session_state.processed_data[column_name].value_counts()
                    ...
                    st.error("Plot type not recognized. Please specify a valid plot (e.g., heatmap, bar, scatter, line, box, histogram, distplot, pie, violin, kde).")
